...
//! # Design notes
//!
//! **Where the pieces are.** This module is the collection and its frozen
//! copies; `src/curate/events.rs` is the surface an Operator runs it from,
//! [`page`] is the page a recipient opens, and [`crate::export`] grew a second
//! `Source` rather than a second pipeline.
//!
//! **The other half of "a snapshot, not a pointer" is the quiet one.** Leaving
//! `event_calls` out of [`crate::db::repo::delete_calls`] is only half of it —
//! forget a child table there and the sweep fails at its oldest marked Call
//! forever (#55's lesson), so the absence is deliberate. The mirror image is
//! that `repo::referenced_object_keys` had to *gain* the frozen keys, or
//! orphan-GC — which exists to delete exactly "an object with no Call row" —
//! silently empties every Event one `orphan_grace` after it was made, and
//! nothing fails until somebody opens one months later.
//! `tests/events.rs::orphan_gc_spares_the_frozen_copies` is what would notice.
//! And while both survive, one transmission is stored twice: that is the price
//! copying pays.
//!
//! **The snapshot is the wire shape, which is what makes it safe to keep for
//! years.** [`crate::call::StoredCall`] gained `Deserialize` + a
//! container-level `default` for this one caller: a field added to the
//! Archive's wire reaches a frozen Call with no migration (#65's manifest
//! rule), and a document written by this release and read by a later one gets
//! the new field's default rather than a parse error. `object_key` is
//! deliberately *not* in it — the snapshot is the Call as it was, the column is
//! where our copy is, and keeping them apart means there is nothing to disagree
//! about. Four facts are columns as well (`call_at_ms`, `duration_ms`,
//! `audio_size`, and the order) because SQL has to add them up: an Event's
//! export measures itself in one statement and the size cap must count frozen
//! bytes without parsing a JSON document per row.
//!
//! **The admin gate was grilled before the first test**, not discovered in
//! review, and is recorded in `docs/adr/0017-events-are-admin-gated.md`: an
//! unauthenticated POST that commits a Pi's SD card for good is what #66
//! refused to build.
//!
//! **When frozen bytes alone exceed the cap**, `retention::sweep` eats the
//! Archive around them and then **stops and says so once** (`frozen_bytes` on a
//! WARN, ADR-0011 rule 8's shape) — the only honest answer when the thing over
//! the cap is the thing the Operator asked to keep forever.
//! `frozen_audio_bytes` is read once per sweep rather than per batch, because
//! pruning Calls cannot change it.
//!
//! **The link's token is a nullable column on `events`** — no second table, so
//! nothing to remember in `delete_calls` (#66's "a column goes with its row") —
//! and `curate::events::token_change` clears it on `shared: false`, because
//! this toggle is the *only* revoke there is and one that merely closed the
//! door would leave a leaked link unkillable. That inconsistency shipped for
//! the length of one test run: the doc comment claimed the token survived a
//! toggle while the code (rightly) killed it, and
//! `tests/events.rs::turning_sharing_off_kills_the_link_for_good` is what found
//! it. There is **no `[events]` section at all**.
//!
//! **A flattened document owns its keys: `id` collided twice, one struct
//! apart.** `EventDetail` flattens an `EventRow` (whose `calls` is a *count*)
//! beside a `members` array — which was called `calls` and silently won,
//! costing the count; and `EventMember` flattens a `StoredCall` (whose `id` is
//! never `skip_serializing_if`) beside a field of its own called `id` — which
//! the Call won, so the client read the Call's id and used it to address a
//! delete. So what sits beside a flattened type is named for what it is
//! (`members`, `memberId`) and never for what that type already spends. There
//! is no `callId` either — it would be a third spelling of `call.id`.
//! `tests/events.rs::a_member_says_which_call_it_was_and_which_member_it_is`
//! freezes the *third* of three Calls so the two ids cannot coincide, because a
//! single-Call fixture has both at `1` and proves nothing.
//!
//! **A create that could not be filled is undone, and a batch too big is
//! refused.** The row and its members are two writes and nothing can make them
//! one — freezing copies *objects*, which no database transaction reaches — so
//! `create` compensates explicitly: an Event named and left empty by a 500 is
//! one an Operator retries, and a retry that accumulates duplicates is worse
//! than a failure that leaves nothing behind. The honest limit is that a
//! database refusing everything about `event_calls` refuses the undo's own read
//! too, which leaves an empty Event on the listing and one WARN — *the only
//! record there is*, because the `Failure` the caller is told about is the
//! freeze's and this one is dropped. And `MAX_PER_REQUEST` is a **constant, not
//! a setting**: `[export] max_calls` bounds a stranger's appetite, where this
//! bounds a Pi copying objects one at a time inside one request.
//!
//! The client half — why `AddToEvent` reports through its parent — is in
