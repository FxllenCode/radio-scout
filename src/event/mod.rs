...
//!
//! **An Event is a copy, and that is what keeps the sweep one pass over one archive (#67, spec US 38).** `src/event/` is a named collection of **Calls** whose audio is **frozen** — copied under a key of its own at curation time — so an incident outlives **Retention**; `src/curate/events.rs` is the surface an Operator runs it from, `src/event/page.rs` is the page a recipient opens, and `crate::export` grew a second [`Source`] rather than a second pipeline. rdio-scanner has nothing like it: its archive is a list you re-find things in, and when the window passes over an incident the incident is gone. Nine things follow.
//!
//! **A member is a snapshot, not a pointer, and the absence of a foreign key is the feature.** `event_calls.call_id` is a plain integer, because the row has to still be there after the sweep has taken the Call it was made from. That is [`share_link`]'s rule exactly inverted: a **Share link** is a child that goes *with* its Call, which is why `repo::delete_calls` must name it — forget one and the sweep fails at its oldest marked Call forever (#55's lesson). An `event_calls` row must **survive** that same delete, so it must not be in that list and must carry no constraint that would make it fail. The other half is the mirror image and is the quiet one: `repo::referenced_object_keys` had to *gain* the frozen keys, or orphan-GC — which exists to delete exactly "an object with no Call row" — silently empties every Event one `orphan_grace` after it was made, and nothing fails until somebody opens one months later. `tests/events.rs::orphan_gc_spares_the_frozen_copies` is what would notice.
//!
//! **Copying is what was bought, and the alternative is what was refused.** Teaching Retention to *spare* a Call some Event holds costs a join on the hot path of every prune, an exemption the size cap would then have to reason about, and a second meaning for `calls.object_key`. The spec settled it before the ticket was written and #66 recorded it while deciding what a **Star** is worth (*Events need no exemption, having copied their audio*). The price is paid once, in bytes, at the moment an Operator says this one matters — and while both survive, one transmission is stored twice.
//!
//! **The snapshot is the wire shape, which is what makes it safe to keep for years.** `crate::call::StoredCall` gained `Deserialize` + a container-level `default` for this one caller: a field added to the Archive's wire reaches a frozen Call with no migration (#65's manifest rule), and a document written by this release and read by a later one gets the new field's default rather than a parse error. `object_key` is deliberately *not* in it — the snapshot is the Call as it was, the column is where our copy is, and keeping them apart means there is nothing to disagree about. Four facts are columns as well (`call_at_ms`, `duration_ms`, `audio_size`, and the order) because SQL has to add them up: an Event's export measures itself in one statement and the size cap must count frozen bytes without parsing a JSON document per row.
//!
//! **It is admin-gated, and that is a departure from the ticket's own story.** Spec US 38 is a **Listener**'s, and a Listener holds no credential — which is exactly why a Star is a *window* the Operator sets and a Share link is bounded by the Calls table. Freezing is bounded by **nothing**: it spends disk permanently and no policy here can reclaim it, which is the feature working. An unauthenticated POST that commits a Pi's SD card for good is what #66 refused to build, so curating lives under `crate::curate` where the gate is a property of the prefix; a Listener still receives, plays and exports a shared Event. This was grilled before the first test rather than discovered in review.
//!
//! **Frozen bytes are counted by the size cap and never taken by it.** They are stored audio on the same disk, so a cap blind to them stops being true the moment an incident is curated — and `max_size_gb` exists to stop an SD card filling. They cannot be *pruned*, so `retention::sweep` eats the Archive around them and, if that is not enough, **stops and says so once** (`frozen_bytes` on a WARN, rule 8's shape): the visible failure rather than the quiet one, and the only honest answer when the thing over the cap is the thing the Operator asked to keep forever. `frozen_audio_bytes` is read once per sweep rather than per batch, because pruning Calls cannot change it.
//!
//! **The link is a toggle, not a window, and turning it off is a revoke.** A Call's link expires because a stranger minted it and a URL in a group chat should not be a permanent endpoint; neither half applies to an Operator publishing an incident that is durable by definition. So the token is a nullable column on `events` — no second table, so nothing to remember in `delete_calls` (#66's "a column goes with its row") — and `curate::events::token_change` clears it on `shared: false`, because this toggle is the *only* revoke there is and one that merely closed the door would leave a leaked link unkillable. That inconsistency shipped for the length of one test run: the doc comment claimed the token survived a toggle while the code (rightly) killed it, and `tests/events.rs::turning_sharing_off_kills_the_link_for_good` is what found it. `[share] enabled` still closes every door at once, and there is **no `[events]` section at all** — an Event borrows `[share]` for its page and `[export]` for its downloads, because those answer the questions an Operator has already been asked.
//!
//! **Two documents and one key: `id` collided twice, one struct apart.** `EventDetail` flattens an `EventRow` (whose `calls` is a *count*) beside a `members` array — which was called `calls` and silently won, costing the count; and `EventMember` flattens a `StoredCall` (whose `id` is never `skip_serializing_if`) beside a field of its own called `id` — which the Call won, so the client read the Call's id and used it to address a delete. The rule that falls out is worth more than either fix: **a flattened document owns its keys**, so what sits beside one is named for what it is (`members`, `memberId`) and never for what the flattened type already spends. There is no `callId` either — it would be a third spelling of `call.id`. `tests/events.rs::a_member_says_which_call_it_was_and_which_member_it_is` freezes the *third* of three Calls so the two ids cannot coincide, because a single-Call fixture has both at `1` and proves nothing.
//!
//! **A create that could not be filled is undone, and a batch too big is refused.** The row and its members are two writes and nothing can make them one — freezing copies *objects*, which no database transaction reaches — so `create` compensates explicitly: an Event named and left empty by a 500 is one an Operator retries, and a retry that accumulates duplicates is worse than a failure that leaves nothing behind. The honest limit is that a database refusing everything about `event_calls` refuses the undo's own read too, which leaves an empty Event on the listing and one WARN — *the only record there is*, because the `Failure` the caller is told about is the freeze's and this one is dropped. And `MAX_PER_REQUEST` is a **constant, not a setting**: `[export] max_calls` bounds a stranger's appetite, where this bounds a Pi copying objects one at a time inside one request.
