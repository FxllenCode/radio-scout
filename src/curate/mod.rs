...
//!
//! `tests/curate.rs` walks every path in [`routes`] looking for a 401, so the
//! "protected by construction" claim above is proven rather than trusted.
//!
//! # The rules every screen here shares
//!
//! - **A refusal is a value with a status**: [`Rejected`] is closed, decides
//!   its own slug *and* its own `StatusCode`, and renders as JSON — the one
//!   `Reason` arm that picks a status, because one surface legitimately answers
//!   400, 404 and 409 and a form needs to tell "you typed something wrong" from
//!   "somebody changed this underneath you". Every arm is DEBUG (ADR-0011 rule
//!   3: an Operator is looking at the form, and a county-scale bulk edit at
//!   WARN would fill a log with their own typing).
//! - **Absent and `null` are different**, read through `nullable` — a `PATCH`
//!   that could not tell them apart could set a label and never clear one, and
//!   the nullable columns #20 shipped (`systems.enhancement`) exist precisely
//!   because *inherit* is a third state. The CSV importer takes the opposite
//!   convention on purpose (#18: a spreadsheet cannot spell the difference and
//!   a form can).
//! - **Retention owns removing Calls**: a delete that would take some is
//!   refused with the count, and `?force=true` borrows `retention::prune`
//!   rather than growing a second row-then-object pass that could strand audio
//!   in a bucket.
//! - **The blacklist has two spellings and one parser** — a toggle on the
//!   Talkgroup row and the raw list on the System form, both through
//!   `systems::blacklist_of`/`blacklist_text`, because an Operator thinks "stop
//!   ingesting this channel" while a Ref whose channel does not exist yet can
//!   only be named on the parent. Renumbering or deleting a channel carries its
//!   blacklisting with it, or the policy inverts silently.
//! - **Groups and Tags are written once** over a `labels::Table` value rather
//!   than twice, and only their *deletes* differ (a Group's links go, a Tag's
//!   pointer is nulled, the Talkgroups always survive). The whole-set write is
//!   `repo::set_talkgroup_groups`, shared with the configuration document
//!   ([`document`]).
//! - **`RADIO_SCOUT_API_KEY` stopped being an authority**:
//!   `startup::provision_ingest_key` registers it only while the roster is
//!   *empty* (a first run, a wiped database) and otherwise reports
//!   `RosterIsCurated`, because a variable that re-asserted itself every boot
