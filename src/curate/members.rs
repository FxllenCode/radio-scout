...
//! # Written once
//!
//! - [`after`] is one generic function serving Refs and Ranges both.
//! - `?dryRun` is read through `import::asked_for_a_dry_run`, written once so
//!   the CSV preview and this one cannot disagree about what the flag means.
//! - **The line about a merge is written once** (`MergeChange::record`) and
//!   **the rule about what a fold may take is written once**
//!   (`repo::member_ref_held_elsewhere`) — both were briefly two copies, and
//!   the log line had already drifted into two field sets under one message,
//!   which is #92's lesson a layer up: a surface's refusal *shape* may differ
//!   (a rejected CSV row names a line, a form gets a status beside an input),
//!   the policy underneath may not.
//!
//! "1 folded, 412 calls" is the same sentence whichever channel went, which is
//! why the preview's per-Ref detail matters. The client refuses to offer a
//! cross-System fold for the same reason as the `recorded` row above; the chain
//! fold it applies is described in
//! `client/src/components/admin/MergeEditor.tsx`.
