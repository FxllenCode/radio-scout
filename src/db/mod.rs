...
//!
//! Applying a migration logs its name at INFO (`db::migrate`, one migration at
//! a time so the line lands after the migration it reports); an already-current
//! schema says so at DEBUG.
