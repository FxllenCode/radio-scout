...
//!
//! Two consequences worth knowing. An **ordering is a value**, so the
//! ack-before-Backfill is `[Send(subscribed), Backfill(cursor)]` in a returned
//! list rather than two statements that could be swapped without a test
//! noticing. And the **access scope is an input** (`Connection::new`), so a
//! restricted connection is something a test can open, on the Backfill path as
//! well as the live one.
//!
//! The **Backfill cursor is an `Emission`** (CONTEXT.md), not a Call id — see
//! ADR-0004's #94 amendment for why storage order is insufficient and what it
//! costs on the wire (`protocol: 2`, a `seq` on every `call` frame, a `gap`
//! frame when a page could not reach back far enough).
