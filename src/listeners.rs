...
//! # Design notes
//!
//! A connection joins the count through `Listeners::arrive`, a guard taken in
//! `handle_socket`, so a connection is a Listener for exactly as long as it
//! runs; the chart is read through `GET /api/admin/listeners`.
//!
//! **The column *set* is what is asserted.** `tests/listeners.rs` checks that
//! `listener_samples` has exactly id, instant and count — not the rows —
//! because that is the only place a fourth column would be noticed: an address
//! or a session added later would pass every other test in the suite and look
//! perfectly ordinary in the Logs view.
//!
//! **The peak is one `fetch_max` on connect**, and the window re-seeds at
//! *whoever is still connected* rather than at zero, or a steady evening
//! listener would be counted once and never again.
//!
//! **It is admin-gated, and it is the only read surface here that is.** The
//! Archive is open because listening is open (ADR-0008); how many people take
//! that up is the Operator's business rather than a fact a Listener gets to
//! publish on their behalf. `Listeners::current()` is the live half, sitting in
//! `AppState` for #70's status page to read.
//!
//! **It ships on**, like `[quiet] enabled` and the Mining sweep and unlike
//! `[enhancement] mode`, because history cannot be recovered afterwards: an
//! Operator who has to find a setting first has already lost whatever happened
//! before they found it. A day is 1,440 rows. `[retention] listener_days`
//! bounds the table on its own window, swept by the sweeper that already sweeps
//! the log — a quarter, because "was last winter busier" is a question about a
//! period whose audio went months ago.
//!
//! **The first tick is not immediate**, which is the one place this differs
//! from the retention sweeper: a sweep at boot catches up on a policy that may
//! have gone unenforced for hours, where a sample at boot is a window nobody
//! was connected for, and an Instance that restarts often would fill its own
//! chart with zeroes it created.
