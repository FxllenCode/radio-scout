...
//!
//! **Two questions, never one: what is sensitive, and who may hear it (#68, spec US 52).** `src/access.rs` is the **Access code** seam ADR-0008 deferred and #94 left `AccessScope` behind for. `systems.restricted` / `talkgroups.restricted` say which channels are gated; a code says who opens them; and deriving either from the other — a channel being gated because some code names it, which is the obvious short cut — makes the state an Operator is in halfway through setting this up **inexpressible**: *sensitive, and no code written yet*, which must be silent rather than open. It would also mean a `*`-scoped code gating the whole Instance by accident, which is rdio-scanner's all-or-nothing PIN wall arrived at sideways. Nine things follow.
//!
//! **An Instance that gates nothing pays nothing, and that is structural.** `Access::is_gating` is one cached bit on `Tones::is_armed`'s terms, and while it is false no read resolves a grant, adds a clause or makes a join — so "an instance with no codes behaves exactly as today" is true by construction rather than by inspection. The bit's two ways of being stale cost very different things (stale-true buys a join, stale-**false** leaves a gate open), so it is re-read on the same request that writes a `restricted` column — `curate::systems`, `curate::talkgroups`, `curate::document`'s importer — and `tests/access.rs` asserts the cost as a **statement count** and the freshness as *the very next request*, rather than trusting either.
//!
//! **A code's scope is a `Selection`**, the live feed's own matrix — which a **Downstream** and a **Webhook** are already scoped by — and three things come free: `Selection::selects` is the per-channel rule, `archive::within` is already the SQL for it (exhaustively tested against that predicate since #63), and `selection::stored` already decides once what an unreadable stored scope means. It reaches **nothing**, which here is the safe direction for a sharper reason than it is for a peer: the alternative is a corrupt row handing a stranger the Operator's whole County. The wildcard is therefore spelled `{"all": true}` rather than by leaving the column empty, so a code created with no scope is not a master key. `AccessScope::granting` normalizes a scope that reaches everything to `All`, which is what keeps a master code costing a search no joins.
//!
//! **Two secrets on one row, because one cannot do both jobs.** An Operator picks a code and says it out loud — a low-entropy human secret, exactly what Argon2id exists for and exactly what `api_keys`' unsalted SHA-256 is wrong for (#51's own note). But Argon2id can never run on the path of an audio range request, so it runs **once**, at `POST /api/unlock`, and what a browser carries afterwards is the row's **grant**: 128 minted bits, kept recoverably the way a **Share link**'s token is and guarded the way a **Webhook**'s URL is. `CodeRow` has no field for either, so no later edit to the listing can start carrying one. The grant is durable and belongs to the *row* — no session table, no restart logging anyone out, one `DELETE` revoking every browser — and **editing the code re-mints it**, because rotating a secret that leaked must not leave the browsers holding the old one connected.
//!
//! **The gate on a Call is its own channel's, whichever channel matched.** A **Patch** puts one transmission on several channels and only the Call's own carries `restricted` in hand, so `StoredCall::restricted` is `COALESCE(talkgroup, system)` read for no statement by the denormalizer that joins both tables anyway — which is what lets a live frame be gated on the same fact `archive::gate` filters on. A Call addressed to a gated channel stays gated however it was patched; one addressed to an open channel stays open, which hides nothing, because that audio is reachable under the open channel's own name by anybody.
//!
//! **Out of scope answers exactly like not there.** `Reason::CallNotFound`, never a `403`: a `403` on a hand-typed id is a working oracle for what an Operator gated, one id at a time. The many-Call reads take the SQL (`gate`, applied last in `filtered_by` and joining only when there is something to apply), and the one-Call reads take `access::reaches_call`, which costs **no statement at all** when nothing is gated. `parse_search` takes a `Viewer` rather than a handler remembering to set one afterwards — a search built from a query string is by definition somebody's, and what it may reach is not theirs to say — while `CallSearch::default` reaches every Call there is, because a hand-built search belongs to a **Worker**.
//!
//! **An HTTP read degrades; the socket refuses.** A browser holding a grant the Operator revoked overnight must fall back to open listening and be **told**, not find the whole app answering `401` — so `Held::Stale` rides on `GET /api/catalog` and the client lets go of what it holds. The live feed is the other case, because it *is* a thing a Listener asked for: a connection over its code's limit is accepted and then sent a `refused` frame and closed, and an expiry is re-read on the connection's own heartbeat (`Event::Tick` carries the clock). rdio does neither — it tells its client `max` or `expired` and **goes on serving it**, because `client.Access = access` is assigned before either check, so an expired code there grants full scoped access.
//!
//! **Two secrets, and neither is ever a log line.** The grant rides the **query string**, which `http_log` writes never (ADR-0011 rule 2) — the **Share link** argument, one credential along — so "a grant is never logged" is true by construction. A refused unlock names its source under rule 5's authentication exemption; a **successful** one deliberately does not, because "10.0.0.7 listens to PD Tac" is the record of who listened that rule exists to stop an Instance accumulating. rdio writes the *presented code* into its log on a failed attempt (`controller.go:462`). The unlock is bounded by `crate::lockout::Lockout` — the admin login's own ledger, lifted out of `admin.rs` and parameterized by a `Budget`, because both guard an Argon2id verification and an unbounded guesser is also a way to exhaust a Pi.
//!
//! **The configuration document carries the gates and none of the codes.** `restricted` is ordinary curation and travels (a restore that dropped it would silently open every gated channel on the Instance it was restored onto — the one direction this must never fail in); an Access code is a secret and is absent entirely, the **Webhook** rule.
