...
//! and #94 left [`AccessScope`] behind as the seam; this is what resolves one.
//!
...
//!
//! # What follows from keeping the questions apart
//!
//! - **Nothing-gated is proven, not trusted.** While [`Access::is_gating`] is
//!   false no read resolves a grant, adds a clause or makes a join. The bit is
//!   re-read on the same request that writes a `restricted` column —
//!   `curate::systems`, `curate::talkgroups`, and `curate::document`'s importer
//!   — and `tests/access.rs` asserts the cost as a **statement count** and the
//!   freshness as *the very next request*, rather than trusting either.
//! - **The empty scope reaching nothing** is safe here for a sharper reason
//!   than it is for a **Downstream** or **Webhook** peer (both also scoped by a
//!   `Selection`): the alternative is a corrupt row handing a stranger the
//!   Operator's whole County. The wildcard is therefore spelled `{"all": true}`
//!   rather than by leaving the column empty, so a code created with no scope
//!   is not a master key. `AccessScope::granting` normalizes a scope that
//!   reaches everything to `All`, which is what keeps a master code costing a
//!   search no joins.
//! - **Two secrets on one row, because one cannot do both jobs.** An Operator
//!   picks a code and says it out loud — a low-entropy human secret, exactly
//!   what Argon2id exists for and exactly what `api_keys`' unsalted SHA-256 is
//!   wrong for (#51's own note). But Argon2id can never run on the path of an
//!   audio range request, so it runs **once**, at `POST /api/unlock`, and what
//!   a browser carries afterwards is the row's **grant**: 128 minted bits, kept
//!   recoverably the way a **Share link**'s token is and guarded the way a
//!   **Webhook**'s URL is. `CodeRow` has no field for either, so no later edit
//!   to the listing can start carrying one. The grant is durable and belongs to
//!   the *row* — no session table, no restart logging anyone out, one `DELETE`
//!   revoking every browser — and **editing the code re-mints it**, because
//!   rotating a secret that leaked must not leave the browsers holding the old
//!   one connected.
//! - **The Call's own gate is read for free.** `StoredCall::restricted` is
//!   `COALESCE(talkgroup, system)`, read for no statement by the denormalizer
//!   that joins both tables anyway — which is what lets a live frame be gated
//!   on the same fact `archive::gate` filters on.
//! - **Out of scope answers exactly like not there.** `Reason::CallNotFound`,
//!   never a `403`: a `403` on a hand-typed id is a working oracle for what an
//!   Operator gated, one id at a time. The many-Call reads take the SQL
//!   (`gate`, applied last in `filtered_by` and joining only when there is
//!   something to apply), and the one-Call reads take [`reaches_call`], which
//!   costs **no statement at all** when nothing is gated. `parse_search` takes
//!   a `Viewer` rather than a handler remembering to set one afterwards — a
//!   search built from a query string is by definition somebody's, and what it
//!   may reach is not theirs to say — while `CallSearch::default` reaches every
//!   Call there is, because a hand-built search belongs to a **Worker**.
//! - **An HTTP read degrades; the socket refuses.** A browser holding a grant
//!   the Operator revoked overnight must fall back to open listening and be
//!   **told**, not find the whole app answering `401` — so `Held::Stale` rides
//!   on `GET /api/catalog` and the client lets go of what it holds. The live
//!   feed is the other case, because it *is* a thing a Listener asked for: a
//!   connection over its code's limit is accepted and then sent a `refused`
//!   frame and closed, and an expiry is re-read on the connection's own
//!   heartbeat (`Event::Tick` carries the clock).
//! - **Neither secret is ever a log line.** The grant rides the **query
//!   string**, which `http_log` writes never (ADR-0011 rule 2) — the **Share
//!   link** argument, one credential along — so "a grant is never logged" is
//!   true by construction. A refused unlock names its source under rule 5's
//!   authentication exemption; a **successful** one deliberately does not,
//!   because "10.0.0.7 listens to PD Tac" is the record of who listened that
//!   rule exists to stop an Instance accumulating. The unlock is bounded by
//!   `crate::lockout::Lockout` — the admin login's own ledger, lifted out of
//!   `admin.rs` and parameterized by a `Budget`, because both guard an Argon2id
//!   verification and an unbounded guesser is also a way to exhaust a Pi.
//! - **The configuration document carries the gates and none of the codes.**
//!   `restricted` is ordinary curation and travels (a restore that dropped it
//!   would silently open every gated channel on the Instance it was restored
//!   onto — the one direction this must never fail in); an Access code is a
//!   secret and is absent entirely, the **Webhook** rule. How the client
//!   attaches the grant is in `client/src/hooks/useGrant.ts`.
