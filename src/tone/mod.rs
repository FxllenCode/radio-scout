...
//!
//! [`detect::runs`] and [`matches()`] are the two halves below; [`worker`] is the
//! errand, and `Tones`/`ToneConfig` are the policy.
//!
...
//!
//! The alternative — asking the audio a question per profile — would decode and
//! transform the same Call twenty times on a county dispatch channel, on the
//! hardware least able to afford it. The audio is asked exactly one question —
//! *where is a steady tone, at what frequency, for how long* — and the answer, a
//! **`Run`**, is a handful of values small enough to print. That is why `matches`
//! has an exhaustive near-miss table where `runs` has a proptest over the whole
//! band.
//!
//! **Two adjacent runs may overlap by up to one analysis window, and the
//! matcher's gap is therefore signed.** The window straddling the moment tone A
//! becomes tone B belongs to whichever fills more of it, so both runs claim part
//! of it — a real Quick Call reads as a *small negative* gap, and a matcher
//! measuring a magnitude would fail every page-out there is. The overlap is
//! asserted rather than tolerated silently.
//!
//! # No speech, ever
//!
...
//! # A Worker of its own
//!
//! Detection is a separate Worker from enhancement (ADR-0019,
//! `docs/adr/0019-tone-detector-is-its-own-worker.md`), and the reason is what
//! ships. The ticket says "the off-path audio worker", but `[enhancement] mode`
//! is `off` by default, so a detection folded into that worker would never run on
//! a stock install. The cost of the split is one extra decode on an Instance
//! running both — which is the cheaper half, since detection decodes and
//! transforms where enhancement decodes, resamples twice, filters, measures
//! loudness and re-encodes. `crate::enhance::decode` is shared rather than
//! re-written, so the audio a page is looked for in is the audio that gets
//! levelled.
//!
//! **The roster gate is one cached bit, and every write re-reads it.**
//! `crate::webhook` keeps its cost off ordinary Calls by asking `Marks::is_empty`
//! about the Call in hand; there is no equivalent here, because whether a Call
//! *might* page can only be answered by asking the database. So `Tones::armed`
//! caches "does any enabled profile exist", refreshed at boot and by
//! `curate::tones` and `curate::document`'s importer on the same request that
//! changed the roster — and the two directions of being wrong cost very different
//! things (stale-true wastes a statement, stale-false misses a page).
//! `tests/tone.rs` asserts end to end that a profile written now applies to the
//! very next Call, rather than trusting it; the document importer's re-arm is a
//! real gap that test caught.
//!
//! **Nothing goes back over the Archive.** `ToneState::NONE` is never re-queued,
//! which is `EnhancementState::NONE`'s rule for its reason: an Operator writing
//! their first profile must not find their whole Archive being re-read on the
//! next boot. What a restart *does* pick up is `pending`, which is work this
//! Instance already accepted and had not finished.
//!
//! # Profiles and marks
//!
//! **A profile is an ordered sequence, and one that could never fire is
//! refused.** A sequence spells a single long group tone, Quick Call II's two,
//! and an A-B-then-group run of three, where a fixed A/B pair would owe a
//! migration the first time somebody used one. `tone::unusable` is the rule — a
//! step outside the band `detect` listens to, a tone too short to measure, a
//! tolerance wide enough to page the neighbouring station — and it is asked by
//! the admin form, the configuration importer *and* `client/src/lib/tone.ts`
//! before submit, on `is_postable_url`'s terms: the failure it prevents produces
//! no error anywhere, because a pager that is not being watched looks exactly
//! like a pager that has not gone off.
//!
//! **The mark records which profile fired, and the label is a snapshot.** An
//! Operator with twelve stations on one dispatch channel is asking *who* was
//! paged. `call_tones` carries the label as it was when the page fired, with no
//! foreign key, so a profile renamed next year does not rewrite last year's pages
//! and one deleted does not erase them — the opposite of a Talkgroup's label,
//! which *is* joined, and the difference is that a page-out is an event where a
//! channel is a thing. **`calls.tone` is one column carrying two questions** —
//! was it looked at, did it hold a page — because the five states are mutually
//! exclusive and total.
//!
//! The stations themselves ride on `StoredCall`, which looks like the array
//! `freqList`/`srcList` are kept off it to avoid, and is not: those are on
//! *every* Call, where a page-out happens a few times a day, so the key is absent
//! from essentially every one. `stored_calls` reads the child table **once per
//! page and only when something in that page carries the mark**, so an ordinary
//! search page and every ordinary live frame still cost exactly the statements
//! they did before. Putting it on `CallDetail` instead was the first shape, and
//! `/code-review` killed it: this client has no call-detail screen, so *which
//! station* would have been reachable from nothing — a badge saying a page
//! happened and no way to learn whose.
//!
//! **Deleting a Call means deleting `call_tones`, and forgetting one is invisible
//! until it isn't.** `repo::delete_calls` names every child table because the
//! foreign keys are `RESTRICT`; a table left out passes every test that stores
//! and reads, and fails the **retention sweep** — which walks oldest-first, so
//! from the moment the oldest marked Call comes due, every sweep fails at the
//! same row forever and an Operator's disk quietly stops being bounded.
//! `curate::talkgroups::remove` owes `tone_profiles` the same way. Both were
//! real, both were found by `/code-review` rather than by the suite, and
//! `tests/tone.rs::a_marked_call_is_still_prunable` is what would notice now.
