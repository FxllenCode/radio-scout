...
//! # The line itself
//!
//! Its fields are `method`, `path`, `status` and `duration_us`, under a span
//! carrying a 16-hex `request_id`. When the peer is trusted, the address
//! believed is the **rightmost entry of `X-Forwarded-For` that isn't itself a
//! trusted proxy**; whether it rides the line is then rule 5's call — ingest
//! lines always, everything else only on a line that is already DEBUG. The live
//! feed logs its upgrade and its connect/disconnect here, never a frame.
