...
//!
//! **Activity is one aggregate over the search's own filters, and the axis is a value (#62, spec US 34–35, 41).** `src/activity.rs` is the time axis — where it starts, how wide a bucket is, how many there are, and which bucket an instant falls in — and `archive::call_activity` and `listeners::read` are two aggregates over it. Six things follow.
//!
//! **The ribbon's filters are the results' filters, not a second set that agrees today.** `call_activity` takes a `CallSearch` and builds through the same `Filters`/`CallQuery` the page and its total are built from, so the bars over a page of results add up to the number above them — asserted as that equality (`tests/archive.rs`) rather than against a table of expected counts. It is also what makes this the per-Talkgroup chart the ticket asks for: a chart of one channel is this read with a Talkgroup filter set, and #63's DVR timeline is this read over a range.
//!
//! **The bucket arithmetic is written twice, in two languages, and they are kept in one file for exactly that reason.** `Axis::index_of` runs in Rust because the client has to place a bucket without asking again; `bucket_expr` runs in SQL because a county's month is half a million rows and nobody sends those over a socket to be counted. `tests/db.rs::assert_activity_buckets` runs the SQL against the Rust **on both dialects**, which is the only thing that can catch them drifting: either alone answers confidently and wrongly.
//!
//! **Integer division is what makes it dialect-safe**, and the filter is what makes the integer division safe. SQLite and Postgres both truncate `bigint / bigint` toward zero, and the aggregate bounds on `[from, to)` explicitly rather than trusting the search's own `after`/`before` — so every offset the division sees is non-negative, truncation *is* flooring, and no row can produce an index off the end of the array. Grouping by `strftime`/`date_trunc` instead would have been a divergence **and** would have put the hour-of-day fold in the server's timezone; the fold is the browser's for that reason, and the server only ever buckets from an origin.
//!
//! **The axis is discovered only when it has to be.** A search naming both bounds has said where its axis is and costs one statement; one naming neither pays a second to ask how far the Archive reaches under those filters. `tests/archive.rs` asserts the difference is exactly one, so a dated search — a preset, a heatmap, a tapped bucket — cannot start paying to be told what it already said. An empty result answers with a flat axis rather than an empty array, so no consumer needs an arm for a series with no buckets in it.
//!
//! And **the grain is bounded rather than refused**: `MAX_BUCKETS` widens a request that would not fit and the response reports the width it actually used, `MAX_LIMIT`'s bargain. That is load-bearing for the heatmap, whose fold into local hours is only exact at hourly grain — so the client bounds its own window (four weeks) instead of asking for a year of hours and folding whatever comes back.
//!
//! **An aggregate groups by the output column's *name*, and #63 is how that was found.** `crate::activity::bucket_group` exists because sea-orm renders `bucket_expr`'s origin and width as **bound parameters**: repeating the expression in the `GROUP BY` emits `(at - $3) / $4` against a select list holding `(at - $1) / $2`. SQLite compares those as equal; Postgres compares parameter *nodes*, decides the grouped expression is a different one, and refuses the whole statement with `column "calls.call_at_ms" must appear in the GROUP BY clause`. So **every chart in the app 500'd on Postgres and worked perfectly on SQLite** from the day #62 landed — the density ribbon, the heatmap and the Operator's listener chart alike — which is exactly the failure the dual-dialect run exists to catch and exactly the failure a SQLite-only loop cannot see. Grouping by the alias sidesteps it: both dialects resolve a bare name in `GROUP BY` against the select list, and there is one expression in the statement rather than two that have to be *recognised* as one. The second half of the same landing is `tests/listeners.rs::samples_taken`, which waited on an **absolute** settle count: the sampler ticks throughout boot, a Postgres boot is two orders of magnitude longer than a SQLite one, so `settled_at_least(2)` was already satisfied before the test had arranged anything — deterministic on each dialect and opposite on the two. Both were red on `next` for two tickets before #63 tripped over them; the lesson is the one [`docs/agents/dual-dialect.md`](docs/agents/dual-dialect.md) already tells, which is to *run it*.
