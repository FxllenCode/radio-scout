...
//! the Listener's.
//!
//! The integer division is safe because of the filter: the aggregate bounds on
//! `[from, to)` explicitly rather than trusting the search's own
//! `after`/`before`, so every offset the division sees is non-negative and no
//! row can produce an index off the end of the array. The hour-of-day fold is
//! the browser's for the timezone reason above, and the server only ever
//! buckets from an origin. The Rust half runs because the client has to place a
//! bucket without asking again; `tests/db.rs::assert_activity_buckets` is the
//! test that holds the two together **on both dialects**.
//!
//! # The ribbon's filters are the results' filters
//!
//! [`crate::archive::call_activity`] takes a `CallSearch` and builds through
//! the same `Filters`/`CallQuery` the page and its total are built from — not a
//! second set of filters that agrees today — so the bars over a page of results
//! add up to the number above them, asserted as that equality
//! (`tests/archive.rs`) rather than against a table of expected counts. It is
//! also what makes this the per-Talkgroup chart #62 asks for: a chart of one
//! channel is this read with a Talkgroup filter set, and #63's DVR timeline is
//! this read over a range.
//!
//! # The axis is discovered only when it has to be
//!
//! A search naming both bounds has said where its axis is and costs one
//! statement; one naming neither pays a second to ask how far the Archive
//! reaches under those filters. `tests/archive.rs` asserts the difference is
//! exactly one, so a dated search — a preset, a heatmap, a tapped bucket —
//! cannot start paying to be told what it already said. An empty result answers
//! with a flat axis rather than an empty array, so no consumer needs an arm for
//! a series with no buckets in it.
//!
//! And **the grain is bounded rather than refused**: [`MAX_BUCKETS`] widens a
//! request that would not fit and the response reports the width it actually
//! used, `MAX_LIMIT`'s bargain. That is load-bearing for the heatmap, whose
//! fold into local hours is only exact at hourly grain — so the client bounds
//! its own window (four weeks) instead of asking for a year of hours and
//! folding whatever comes back.
//!
//! # An aggregate groups by the output column's name (#63)
//!
//! [`bucket_group`] exists because sea-orm renders [`bucket_expr`]'s origin and
//! width as **bound parameters**: repeating the expression in the `GROUP BY`
//! emits `(at - $3) / $4` against a select list holding `(at - $1) / $2`.
//! SQLite compares those as equal; Postgres compares parameter *nodes*, decides
//! the grouped expression is a different one, and refuses the whole statement
//! with `column "calls.call_at_ms" must appear in the GROUP BY clause`. So
//! **every chart in the app 500'd on Postgres and worked perfectly on SQLite**
//! from the day #62 landed — the density ribbon, the heatmap and the Operator's
//! listener chart alike — which is exactly the failure the dual-dialect run
//! exists to catch and exactly the failure a SQLite-only loop cannot see.
//! Grouping by the alias sidesteps it: both dialects resolve a bare name in
//! `GROUP BY` against the select list, and there is one expression in the
//! statement rather than two that have to be *recognised* as one.
//!
//! The second half of the same landing is `tests/listeners.rs::samples_taken`,
//! which waited on an **absolute** settle count: the sampler ticks throughout
//! boot, a Postgres boot is two orders of magnitude longer than a SQLite one,
//! so `settled_at_least(2)` was already satisfied before the test had arranged
//! anything — deterministic on each dialect and opposite on the two. Both were
//! red on `next` for two tickets before #63 tripped over them; the lesson is
//! the one `docs/agents/dual-dialect.md` already tells, which is to *run it*.
