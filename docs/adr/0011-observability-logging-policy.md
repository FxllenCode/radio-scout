...
3. **Every rejected ingest logs at WARN with a machine-readable `reason`** — `invalid-api-key`, `duplicate`, `blacklisted`, `no-talkgroup`, `not-populated`. A Call that does not become a row must leave a line saying why. This is the rule the 2026-07-25 incident bought. **Structural since #92** (below): the line is emitted by the only thing that can turn a `Reason` into a response.
4. **Every 5xx logs at ERROR with the cause and a request id**, and the response body carries **only** that id — never the cause. Internals do not travel to clients or into pasted logs. The rdio-compatible strings for *known* outcomes (`Call imported successfully.`, `duplicate call rejected`, `Incomplete call data: no talkgroup`) are a wire contract and stay byte-identical. (Called a *ref* until #92, which renamed it for the collision with the glossary's **Ref**.)
...
**The third class is *authentication attempts*, not one handler (amended by #70).** It was written as `admin::login`'s lines because that was the only credential an Instance had; there are now three, and each refused attempt earns a source for the same reason. The exemption therefore covers, and covers only:

| Refusal | Why an address |
| --- | --- |
| `invalid-password`, `admin-locked-out` (#19) | somebody is guessing the admin password |
| `invalid-access-code`, `unlock-locked-out` (#68) | somebody is guessing an **Access code** — and the code itself is never written down, which is exactly what rdio-scanner does write (`controller.go:462`) |
| `invalid-metrics-token` (#70) | an Operator's own monitoring has quietly stopped seeing this Instance, which is otherwise a silence they have to guess at |

Three things still bound it, unchanged. A *successful* authentication names nobody — "10.0.0.7 unlocked PD Tac" is the record of who listened this rule exists to prevent. The exemption is the **refusal line's** alone and never the request line's beside it. And the boundary is structural rather than remembered: `Refusal::attempted_from` is the only way an address reaches one, and `failure.rs`'s own table asserts that no other arm carries the field at all.
6. **Messages are static strings; the variable part is structured fields.** `warn!(reason = %slug, "request refused")`, never a formatted sentence. This is what makes a log searchable, a DB sink sane, and rdio's quoting bug impossible. **Structural since #92**: there is one refusal callsite in the process, so "which fields, rendered how" is not a per-handler decision any more.
7. **Levels mean something specific:** ERROR = an operator must act. WARN = something was rejected or dropped **and an operator would want to know** — every ingest rejection, a malformed push subscription, a refused login. INFO = a notable normal event (startup, ingest outcome, one line per request). DEBUG = per-asset and per-range requests, listener IPs, protocol detail — *and* a refusal that is ordinary traffic rather than news (amended by #92: a Call that isn't there, a mistyped filter, an expired admin session, a `/api/push/key` poll on an instance with push off). The request log already emits a WARN line for every 4xx, so a second one per range request would be rule 8's hot loop.

   The dividing line is rule 7's own first clause read strictly: **would an Operator act on it?** A recorder uploading into a blacklist is something they configured and may want to revisit; a browser asking for a pruned Call is not.
...
### Rejections, failures, and the request id

The shape rules 3, 4 and 6 take in the code (#29, **reshaped by #92 — shipped**):

**A handler returns a result, not a response.** `Result<T, failure::Failure>`, where `Failure` is `Refused(Reason)` or `Broke(Stage, cause)` — the two kinds of not-answering, which are owed different things. No handler names a status code, a JSON wrapper or a log macro; the wire shape of a success is decided beside its own type (`impl IntoResponse for Catalog`), and everything about a failure is decided by the arm it names.

**Two closed vocabularies, deliberately not one.** A **Stage** says where the code was when a 5xx happened — Operator diagnostics, kept fine-grained (`dedup`, `store-call`, `search-calls`). A **Reason** says what the caller did wrong (`duplicate`, `no-talkgroup`, `csrf-mismatch`). Conflating them would make #70's "rejections by reason" metric ambiguous between *an Operator must act* and *a Recorder sent something we refused* — which is the split rule 7's levels exist for — and would put unvalidated cardinality behind a metric label.

**Rules 3 and 6 became structural.** Before #92 the rejection funnel was a **convention**: four funnels in three modules, each with its own message, and a new rejection path that built its own response simply skipped the line. It had already drifted — one funnel rendered `reason` bare with a comment explaining why that matters for grepping, another rendered it quoted, and a test pinned the quoted form. Now one exhaustive match on `Reason` yields the slug, the level, the status and the body together, and `impl IntoResponse for Reason` is the *only* way a refusal becomes a response — so a refusal that does not log is not expressible, and a new arm that decides none of those four things does not compile. One static message (`request refused`) with the identity in `reason=`, and the extra fields rule 5's third class permits (`client_addr`, `failures`, `retry_after_secs`) carried as `Option`s that record nothing when absent — so a line about a duplicate has no empty address on it.

**The vocabulary is one string, not two.** For the rdio 417 family the slug *is* the wire detail with its dashes spelled as spaces (`no-talkgroup` ⇄ `Incomplete call data: no talkgroup`), so the string an operator greps and the string a recorder branches on cannot drift apart. Slugs are rendered bare rather than quoted, for the same reason the request line renders its path bare.

**The 5xx redaction was already structural and stays where it is** — applied by the request middleware to any server-error status whatever produced it, including one nobody wrote a cause for (which logs `server error with no recorded cause` — loudly, because a 500 nobody wrote down is the 2026-07-25 failure mode itself). Deliberately *not* moved into the response conversion, which would only cover the 5xxs we build.

**The client gets `internal error (request id: <id>)` and nothing else**, at every 5xx, from every route — #28's request id, which the response also carries as `x-request-id`. The cause travels to the log as `cause=`, never to the body. This costs the operator who only reads their recorder's log the detail they used to get; the id buys it back, and it is the difference between "the server knows" and "the recorder knows and the server doesn't". #92 renamed it off *ref*: `CONTEXT.md` spends **Ref** on the external identifier a recorder sends, and the header, the span field and the type had all called this a request id since #28 — the body was the only thing left saying otherwise.

**One upload is one span** (`ingest{system_ref, talkgroup_ref, call_id}`), nested inside the request span, with the Call id recorded onto it after the row exists — so a rejection line carries the System and Talkgroup without repeating them, and a stored Call's line carries the id it became. Like the request span it exists at ERROR level: the lines that need its context most are WARN rejections, which an operator may be watching with everything else turned down. The span starts where an upload first *has* an identity, so the malformed-body rejections that precede it carry the request id alone — which is all that is known about them. A `Failure` captures the span it was built in and re-enters it to write the line — for a refusal as much as for a 500 (#92), because axum renders a handler's return value after every span it ran in has closed, and on a dropped Call that line is the only record there is.
...
The floor is exactly as wide as rule 5 is, and no wider: the addresses this rule *permits* — a recorder's on an ingest line at INFO, and a refused **authentication attempt's** source at WARN (the admin password, an **Access code**, a metrics token) — are therefore **stored**, for `log_days`. That is deliberate and is most of what the surface is for: "which host stopped uploading" and "somebody is guessing my password, from here" are the two questions an operator with no shell most needs answered, and neither is a record of who listened. Both are asserted in `tests/logs.rs` beside the listener case, so the boundary is a decision rather than a coincidence.
...
## Amendment (#107, 2026-08-16): two worked examples died; the rules did not

[ADR-0014](0014-no-notifications.md) removes Web Push, and this ADR used it to illustrate three things. All three rules stand exactly as written — only the examples are gone, and they are replaced here so the rules keep their teeth.

- **Rule 5's hardest case.** The push *endpoint* was the sharpest illustration of "never above DEBUG": a stable per-device identifier, worse than an IP, which reqwest's own `Display` would have carried into a log line through the back door had the transport error not been rendered through `without_url()`. Nothing in the product is now quite that dangerous — the rule's live subjects are listener IPs and the **Downstream** peer key, which is stored recoverably (a hash cannot be POSTed) and must never be logged in any form. The back-door lesson generalises and is worth keeping: an error type's `Display` is part of what a log line says.
- **Rule 7's WARN exemplar** was "a *malformed* push subscription". The equivalent survivors are the ingest rejections — `invalid-api-key`, `blacklisted`, `no-talkgroup` — which is where an Operator actually looks.
- **The #92 amendment's DEBUG exemplar** was "a `/api/push/key` poll on an instance with push off — a refusal nobody acts on, once per page load". The route no longer exists. The surviving instances of that shape are the admin guard and a Call that isn't there: refused, logged, and nothing an Operator would do anything about.
