...
**Audio enhancement ([ADR-0006](../adr/0006-optional-rust-native-audio-enhancement.md), as amended by #20):** opt-in, off by default, gated at **runtime** — `[enhancement]` in TOML for policy, `systems`/`talkgroups` columns for scope (US 34) — *not* cargo-feature-gated, because a build-time gate is unreachable for operators running a prebuilt binary. Pipeline: symphonia decode → rubato 48k → nnnoiseless denoise → biquad band-pass → ebur128 two-pass loudnorm → **WAV 8 kHz 16-bit mono** (Opus-in-Ogg option with #23). **No AAC encoder ships**, so the published binary stays 100% pure Rust with zero patent surface; measured against real scanner audio, WAV at 8 kHz is already 2.5× smaller than what recorders send. Runs off the ingest path in a **bounded enhancement queue**, swapping the audio object under a stable URL once done; ingest still stores, inserts and returns `200` before any of it starts, so a `200` still means the Call is durable. Denoise benefit on digital audio still to be validated by listening test.
