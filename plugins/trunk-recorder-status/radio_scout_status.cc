// Radio-Scout's Trunk Recorder **status** plugin (#71, spec US 50).
//
// A fork of Trunk Recorder's own `plugins/stat_socket`, which is in the
// recorder's source tree and is **not compiled by it**: the top-level
// `CMakeLists.txt` adds five plugins by name and stat_socket is not one of them
// (`CMakeLists.txt:384-389`). So a stock recorder cannot dial a status socket at
// all, whatever `statusServer` says, and shipping this is the only way an
// Operator gets the dashboard without hand-porting a file out of somebody's
// source tree.
//
// **The wire is unchanged**, deliberately: Radio-Scout's `/api/recorder-status`
// endpoint is written against Trunk Recorder's own status dialect, so a fork
// that "improved" the JSON would only mean an Operator running upstream's
// stat_socket saw less. Three things *are* different, and each is a defect in
// the original:
//
//   1. **Its own `server` key.** Upstream reads the global `statusServer`, of
//      which there is exactly one — so feeding Radio-Scout meant giving up
//      whatever else was reading it. This plugin takes `server` in its own
//      plugin object and falls back to `statusServer` when it is not given,
//      which is how an Operator runs it beside an existing consumer. It is the
//      same argument the rdio uploader plugin already makes for a second
//      `rdioscanner_uploader` entry.
//   2. **Initialised members.** Upstream's constructor sets `m_open`, `m_done`
//      and `m_config_sent` and leaves `m_reconnect`, `retry_attempt` and
//      `reconnect_time` uninitialised — and `poll_one()` reads `m_reconnect` on
//      every pass from the moment the plugin starts, long before anything has
//      written it. Whether a fresh recorder immediately tries to reconnect a
//      socket it has never opened is, upstream, whatever was on the stack.
//   3. **A capped backoff.** Upstream's retry delay is `6 * attempts + rand()`,
//      which grows without bound: a recorder that spent a night unable to reach
//      the far end is, by morning, waiting hours between attempts. This caps it,
//      so an instance coming back up is found within the minute.
//
// Everything else — the message shapes, the field names, the three-second
// cadence — is upstream's, because it is the contract.
...
class Radio_Scout_Status : public Plugin_Api {
  ...
  Config* config;
  /// The URL from this plugin's own object, or empty to fall back to the
  /// recorder's global `statusServer`.
  std::string configured_server;
 ...
 */
  ...
  Radio_Scout_Status()
      : retry_attempt(0), reconnect_time(0), m_reconnect(false), m_open(false),
        m_done(false), m_config_sent(false) {
    ...
    m_client.set_open_handler(bind(&Radio_Scout_Status::on_open, this, _1));
    m_client.set_close_handler(bind(&Radio_Scout_Status::on_close, this, _1));
    m_client.set_fail_handler(bind(&Radio_Scout_Status::on_fail, this, _1));
    m_client.set_message_handler(bind(&Radio_Scout_Status::on_message, this, _1, _2));
  }
  ...
  int send_object(boost::property_tree::ptree data, std::string name, std::string type) {
    ...
    // std::cout << stats_str;
    return send_frame(stats_str.str());
  }
  ...
  void open_socket() {
    ...
    const std::string server = this->configured_server.empty()
                                   ? this->config->status_server
                                   : this->configured_server;
    if (server == "") {
      return;
    }
    ...
    if (ec) {
      m_client.get_alog().write(websocketpp::log::alevel::app, "open_socket: Get WebSocket Connection Error: " + ec.message());
      return;
    }
  ...
  // When to try again, **capped**. Upstream grows this without bound, so a
  // recorder that spent a night unable to reach the far end is waiting hours
  // between attempts by morning — and an instance that came back up half an
  // hour ago is still not being talked to. The jitter is upstream's, and is what
  // keeps a rack of recorders from reconnecting in lockstep.
  long schedule_retry() {
    const long MAX_BACKOFF_SECS = 60;
    retry_attempt++;
    long backoff = 6 * retry_attempt;
    if (backoff > MAX_BACKOFF_SECS) {
      backoff = MAX_BACKOFF_SECS;
    }
    long delay = backoff + (rand() % 30);
    reconnect_time = time(NULL) + delay;
    return delay;
  }
  ...
  void on_close(websocketpp::connection_hdl) {
    ...
    m_config_sent = false;
    m_reconnect = true;
    long reconnect_delay = schedule_retry();
  ...
  int setup_config( std::vector<Source *> sources, std::vector<System *> systems) {
    ...
    // Factory method
    static boost::shared_ptr<Radio_Scout_Status> create() {
        return boost::shared_ptr<Radio_Scout_Status>(
            new Radio_Scout_Status()
        );
    }

 // The plugin's own `server`, so this can run beside whatever else is reading
   // the global `statusServer`. Absent, `open_socket` falls back to that.
   int parse_config(json config_data) {
     this->configured_server = config_data.value("server", "");
     return 0;
   }
   int stop() { return 0; }
...
// `create_plugin` is the symbol Trunk Recorder imports by that exact name
// (`plugin_manager.cc:23-27`); a plugin it cannot find it in is one that loads
// and does nothing.
BOOST_DLL_ALIAS(Radio_Scout_Status::create, create_plugin)
