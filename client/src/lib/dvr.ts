/**
 * The **DVR** (#63, spec US 39–40): one talkgroup or a **Selection**, a range
 * of time, and where in it the Listener is.
 *
 * Pure — a `URLSearchParams` in and a value out, or the reverse, plus the two
 * searches that value implies. The screen (`routes/DvrScreen.tsx`) owns *when*
 * any of it changes; this owns what it means. No product has a DVR —
 * rdio-scanner's time travel is the Previous button, one page of results at a
 * time.
 *
 ...
 * already.
 *
 * **So the serving criterion is met by there being no serve path.** The
 * "playlist with discontinuity markers" is the ordered page of Calls the
 * archive already answers with, each carrying the URL of an object already
 * being served: no stitching, no concatenation, no transcoding, because nothing
 * new is served at all. A real HLS playlist was the other reading and is wrong
 * — a WAV is not a valid HLS segment and Trunk Recorder uploads WAV by default,
 * and ADR-0005's revised ladder already puts MMS out of reach because anything
 * making it viable makes the **Station stream** (#74) strictly better.
 * [ADR-0005's #63 amendment](../../../docs/adr/0005-client-audio-media-session-background.md)
 * is the argument; what a Listener hears as *gapless* is #14's prefetch plus
 * #59's Quiet-span trim taking out the recorder's own dead air.
 *
 ...
 * alone (#89's separation). That is also why `DensityRibbon` takes a **resolved
 * bucket** rather than an offset and an ordering: two screens place the marker
 * from two different facts — `bucketOfOffset` for a page, `bucketOfInstant` for
 * a playhead — and the component keeps knowing nothing about what a position
 * means.
 *
 * # One scoping rule
 *
 ...
 * until a patch is running — a DVR of a channel that was patched that night
 * would go silent exactly when the county was busiest.
 *
 * The server answers that question with a **second implementation**, since a
 * predicate over one Call cannot be a `WHERE` clause (`within` in
 * `src/archive.rs`, whose header says how the matrix becomes SQL);
 * `tests/archive.rs::the_sql_filter_answers_what_the_selection_itself_would` is
 * what holds the two together, enumerating the matrix *exhaustively* (two
 * Talkgroups and the wildcard, each absent/on/off, times the global default —
 * 54 scanners) rather than proving four arms with four hand-picked strings.
 * This is #62's bucket arithmetic one module along, and the reason is the same:
 * either implementation alone answers confidently and wrongly.
 *
 * **The link spelling is one format in two languages, and neither side's own
 * round trip can prove it.** `lib/selectionEncoding.json` is a table both
 * `lib/selectionUrl.ts` and `src/selection.rs` are held to — #62's
 * `assert_activity_buckets` reasoning, without a runtime bridge to share. A
 * drift there is not a crash: the server answers a DVR with a perfectly
 * ordinary page of somebody else's channels. Building it found one real
 * divergence — the client normalised a System's Ref through `Number()` and left
 * a Talkgroup's raw, so a hand-typed `05` addressed a key nothing would ever
 * match — fixed rather than pinned.
 *
 * # Speed, trim and seeking
 *
 * **The speed and trim are Catch-up's levers and deliberately not Catch-up.**
 * CONTEXT.md reserves that word for draining the *listening queue*, which "ends
 * when the queue is empty"; a DVR ends at the end of a range. So there are two
 * flags — `live.catchup` and `playback.hurrying` — and one question,
 * `selectIsHurrying`, asked of the **transport**, because the transport is
 * already the module that knows which source owns the element. One flag would
 * mean a Listener catching up finding a DVR at 1.5×, and a DVR left hurrying
 * speeding up the live feed it hands back to. It is cleared by a **reducer
 * wrapper** when the Run ends rather than in six reducers, which is #59's own
 * trick one slice over: a rate left set would follow the Listener to a Search
 * result they then hear fast with no control on screen to undo it.
 *
 * **Seeking inside a Call is an intent, not a position.** `transport.seek`
 * carries a second *and a nonce*, because asking for the same second twice is
 * two requests; the player applies it keyed on the nonce alone, or turning the
 * speed up mid-Call would drag the Listener back to wherever they last
 * scrubbed. `sourceChanged` clears it, or a seek would land on the next Call —
 * which in a DVR is a different minute of a different hour. The element then
 * range-requests what it lacks, which `src/serve.rs` has answered since #10.
 * The lock screen's `seekto` stays **unbound**: `lib/mediaSession.ts`'s
 * recorded reason — a Call is seconds long — survives a DVR, because a DVR
 * still plays one Call at a time. **The iOS gate re-opens**, as it does for
 * every change to media-source behaviour: research §14 gains **Step 10** (a
 * scrub across Calls changes `src`, which Step 9 does not cover).
 */
