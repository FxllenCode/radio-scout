 ...
 * than two features. Its siblings: `lib/selectionUrl.ts` is the same round trip
 * for a **Selection**, `lib/dateRange.ts` is the four presets, and
 * `lib/share.ts` is how either leaves the app. rdio-scanner addresses nothing
 * but its `?id=` Profile, so a search there is unbookmarkable, unshareable, and
 * gone the moment you look at something else.
 *
 ...
 * holds no filter state at all — it reads `useSearchParams` and writes it — so
 * a reload, the back button, a tab switch and a link somebody sent are one
 * mechanism rather than four features that each have to remember the others.
 * The consequence that pays for it: the Run is told the search changed from
 * **where the change is observed**, in one effect, rather than at every control
 * that could cause one — which is the only way the back button gets the same
 * treatment as a dropdown (#92's argument one layer up). That effect compares
 * with [`sameSearch`] rather than by reference, because `filters` is rebuilt
 * whenever any of the URL moves and a review pass found the Run being told the
 * search had changed on a *page turn*.
 *
 * **A URL is typed by strangers, so everything read out of one is checked** —
 * `lib/persist`'s rule, reaching one layer further out (the `NaN` case is
 * below). A *search* drops the filter it could not read and keeps the rest,
 * because a bad filter costs that filter; a *Selection* link is refused
 * **whole**, because it is one statement made by somebody else and half of
 * their scanner is not a thing they meant to send.
 *
 * **A preset resolves to instants, and the matrix is what a Selection link
 * carries.** `when=today` would mean something different tomorrow, so what
 * reaches the URL is `after=`/`before=` in milliseconds and a link is a *view* —
 * which is what "any view is a bookmark" asks for. The calendar presets are
 * built from local date parts rather than by subtracting 86,400,000 ms, because
 * a day is not always that long, and `before` is exclusive-by-one-millisecond
 * so today and yesterday share no instant. On the other side, a Selection
 * travels as the whole ADR-0004 matrix rather than as a list of what is on:
 * auto-populate (#8) mints Talkgroups the recipient's catalog has and the
 * sender's does not, and only the matrix answers *what to do about a Talkgroup
 * nobody has decided on* the same way at both ends (`lib/selection`'s rule 2,
 * reaching one more place).
 *
 * **Opening a Selection link carries the way back, and a Call link does not
 * need one.** A `?sel=` replaces something the Listener built by hand without
 * their having touched it, which is the **Avoid** undo's case exactly (#58) —
 * same shell, same deadline-as-a-moment, `SELECTION_UNDO_MS` longer than an
 * Avoid's because the mistake is noticed by *reading the panel* rather than by
 * hearing silence. The **Hold** and the **Avoids** are deliberately untouched
 * by a link — they are this Listener's reading of the moment, and they are
 * layers *over* the Selection, so they keep applying to whatever it becomes.
 *
 * **Both links are *consumed*: acted on, then taken out of the address bar,
 * replacing rather than pushing.** What stays in the URL is what describes the
 * screen — the filters — and neither of these does: one is folded into state
 * the browser goes on remembering, the other is how the screen was opened. The
 * failure that proves it is the Call's, and `/code-review` found it: the shell
 * remembers where the Search tab was (including its query), so a `?call=` left
 * in meant the Call was re-played, interrupting whatever was on, **every time
 * the Listener came back to the tab** — a guard ref made it once per *mount*,
 * which is not once per Call. Replacing rather than pushing matters for the
 * same reason: pushing leaves the link one Back away from firing again. A Call
 * that is **gone** is deliberately *not* consumed, because nothing was acted on
 * and the sentence saying so has to survive the next render.
 *
 * **A dropdown earns a history entry; a keystroke does not.** `updateFilters`
 * names the control being typed into, so the first keystroke in a field pushes
 * — Back returns to the search *before* the typing — and the rest replace.
 * Without it, typing a four-digit unit Ref left four entries and Back walked it
 * digit by digit, which is the acceptance criterion ("back/forward work")
 * failing on the one control a Listener types into most.
 *
 * **One hook mints every link** (`hooks/useShareLink`), because three controls
 * on two screens each owe the same three things: reach the platform, say what
 * happened, and take the saying back down. That last one had already been
 * forgotten once — the Talkgroups copy carried the notice and not the timer, so
 * "Link copied." sat under its panel bar for the rest of the session. #92's
 * rule, one layer up: a surface's shape may differ, the policy underneath may
 * not.
 *
 * **Playing forward from a row is a Run configured differently, not a second
 * kind of playback** — CONTEXT.md says so, and it is one lazy query plus the
 * `startRun` that already exists, anchored `{sort: 'oldest', after:
 * call.timestamp}`. The list on screen is untouched, which is the separation
 * #89 named `Run.from` and `windowOffset` apart for.
 *
 * Two smaller things the build had to answer: the date inputs are
 * **controlled**, so they keep the *text* and re-derive only when the bound
 * shown is not the one the text already means (`2026-07-25` is a valid instant
 * — at UTC midnight — so a half-typed date that parses would otherwise be
 * rewritten under the cursor mid-word); and the Search **tab** remembers where
 * it was, in the shell rather than in the tab bar, because a bar linking at a
 * bare `/search` throws away eight taps of filters every time a Listener looks
 * at Talkgroups.
 */
