 ...
 *
 * The CSRF token is attached in **`prepareHeaders`**, on mutations only, read
 * from the session already in the RTK Query cache: a mutation added later
 * inherits it instead of remembering a header, which is exactly the class of
 * omission #18 had to be shipped around on the server.
 *
 * Three more things every admin screen owes:
 *
 * - **A refusal is rendered from the server's own sentence**
 *   (`lib/curateError.ts`, and [`FailureNote`] below) rather than re-derived, and
 *   `refusedForCalls` gates the offer to force on the *slug*, never the status,
 *   because a name collision is also a 409 and a "delete it anyway" button
 *   there would do nothing.
 * - **A form's buttons need `type="button"`**: `Button` renders a bare
 *   `<button>`, so "Deselect" and "Clear their tags" were submitting the bulk
 *   action until a test caught it.
 * - **Changing a filter drops the selection**, because a bulk action over rows
 *   that scrolled out of the answer is the one thing multi-select must never do.
 */
