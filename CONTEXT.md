...
**Window** is the noun for how long one Call is kept, once those overrides have had their say (#69): a **Talkgroup**'s, else its **System**'s, else the Instance's `[retention] days`. `0` is *for good* at every level, which is what makes it **not a floor** — an Operator keeping one System for good can still bound one chatty channel inside it, and a channel can be kept for good inside a System that is not. A Window answers *when* a Call is due rather than *whether* it has aged out, because two policies ask: the age pass takes what is due, and the **size cap** takes what is due **soonest**. With nothing overridden anywhere, due-soonest *is* oldest-first — adding the same window to every Call cannot reorder any of them — which is why the cap's behaviour on an ordinary Instance is unchanged. Where something is overridden it is the difference between an override meaning something and an override being silently worthless: a cap that ate the 90-day System first, because its Calls are the oldest there are, would undo exactly what the Operator asked for.
