...
**Still owed by CI**, deliberately, and tracked rather than forgotten: a **native-arm64 `cargo nextest run`** job so the Pi target is tested and not only compiled (#22 compiles `aarch64-unknown-linux-gnu`, nothing runs it); a **real-S3 (MinIO/Garage)** job for ADR-0002's storage half (#35); and the **Vitest Browser Mode** layer (#34).
...
- **The file** is sectioned — `[server] [database] [storage] [storage.s3] [retention] [ingest] [admin] [log]` — found via `--config`, then `RADIO_SCOUT_CONFIG`, then the working directory. **No file is not an error**: zero-config first run creates `base_dir`, a SQLite database and a filesystem audio store, and serves (spec US 35).
...
- **Two credentials stay out of the TOML entirely** because first run *writes* them: `RADIO_SCOUT_API_KEY` and — since #19 — `RADIO_SCOUT_ADMIN_PASSWORD`, both generated into `.env` (`0600`) with only the path logged. Radio-Scout ships **no default admin password**; rdio ships a known one behind a nag. Everything about the admin surface that is a knob rather than a secret is `[admin]`: `session_idle_secs` (refreshed by use), `session_max_secs` (never), `lockout_attempts`, `lockout_secs`.
