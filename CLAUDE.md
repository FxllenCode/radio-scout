...
**Still owed by CI**, deliberately, and tracked rather than forgotten: a **native-arm64 `cargo nextest run`** job so the Pi target is tested and not only compiled (#22 compiles `aarch64-unknown-linux-gnu`, nothing runs it); a **real-S3 (MinIO/Garage)** job for ADR-0002's storage half; **branch protection** making the hard checks required, which is a repository setting rather than a file and needs the pipeline green once first; and the **Vitest Browser Mode** layer (#34).
...
- **The file** is sectioned — `[server] [database] [storage] [storage.s3] [retention] [ingest] [log]` — found via `--config`, then `RADIO_SCOUT_CONFIG`, then the working directory. **No file is not an error**: zero-config first run creates `base_dir`, a SQLite database and a filesystem audio store, and serves (spec US 35).
...
- **Strict validation.** An unknown key, an unparseable value from any layer, missing S3 credentials for `storage.backend = "s3"`, a zero `retention.batch_size`, a non-positive `max_size_gb`, a negative dedup window, or log directives `tracing` can't parse — each **refuses to boot**, exits `2`, and names the source, the value and what was expected. rdio silently ignores both the unknown key and a file that fails to load. (`1` means the start itself failed, e.g. a bound port.)
